...
- **几何级视窗同步协议 (LayoutManager)**：主进程与渲染层外壳像素级边界严丝合缝，窗口拉伸与分屏无白边与撕裂。
...
- **开箱即用预设模板**：首次启动自动初始化【工作主舱】、【个人空间】、【跨境出海】与【极客沙箱】4 组预置容器。
- **容器设置抽屉 (Container Inspector)**：从左侧栏一键呼出，自由调整容器主题色（12 色色盘）、图标、代理测试与一键清洗缓存。
...
### ⚡ 极客快捷键与会话恢复引擎
- **全键盘快捷键配置页面 (Shortcuts Settings)**：可视化按键捕获器 (Key Recorder)、冲突警示检测、一键重置与 JSON 持久化，所有视窗与容器操作皆可全键盘驱动。
- **会话持久化与轻量唤醒 (Session Restore)**：自动记录全容器与标签状态，重启后后台标签以休眠态（Hibernated）极速唤醒，内存开销为 0。
...
- [📖 核心功能需求与系统设计规范 (PRD v1.4.0 终审版)](specs/functional_specification.md)
